...
import { Sun, Plus, Trash2, Thermometer, Heart, CalendarCheck, DollarSign, Clock } from 'lucide-react';
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-200 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">{t('events')}</h2>
            <p className="text-sm text-slate-500">{t('assign_event')}</p>
          </div>
          <CalendarCheck className="w-8 h-8 text-indigo-100" />
        </div>
        ...
        <ul className="divide-y divide-slate-200">
          {events.length === 0 ? (
            <li className="p-6 text-center text-slate-500">{t('no_events')}</li>
          ) : events.map(ev => (
            <li key={ev.id} className="p-4 sm:px-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-4">
              ...
              <button onClick={() => removeEvent(ev.id)} className="text-slate-400 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition"><Trash2 className="w-5 h-5" /></button>
            </li>
          ))}
        </ul>
      </div>
